...
import re
from brave.api.schemas.pipeline import DownloadStore, FileConetent, InstallComponent, PageComponentRelationQuery, PagePipelineQuery, PublishComponent, PublishRelation, SavePipeline,Pipeline,QueryPipeline,QueryModule, SavePipelineComponentsEdges,SavePipelineRelation,SaveOrder
...
@pipeline.post("/download-store",tags=['pipeline'])
@inject
async def download_store(downloadStore: DownloadStore,
                         sse_service:SSESessionService = Depends(Provide[AppContainer.realtime_service]) ):
    settings = get_settings()
    store_path = f"{settings.STORE_DIR}"
    url = downloadStore.url
    force = downloadStore.force
    filename = url.split("/")[-1]
    # if not filename.endswith(".git"):
    #     raise HTTPException(status_code=500, detail=f"Only support git repo url!")
...
@pipeline.post("/git-stop", tags=['pipeline'])
@inject
async def git_stop(downloadStore: DownloadStore,
                   sse_service:SSESessionService = Depends(Provide[AppContainer.realtime_service]) ):
    settings = get_settings()
    store_path = f"{settings.STORE_DIR}"
    url = downloadStore.url
    ...
    # Repo.clone_from(
    #     "https://github.com/pybrave/enrichment-analysis.git",
    #     "enrichment-analysis"
    # )
                        
@pipeline.post("/git-pull", tags=['pipeline'])
@inject
async def git_pull(downloadStore: DownloadStore,
                   sse_service:SSESessionService = Depends(Provide[AppContainer.realtime_service]) ):
    settings = get_settings()
    store_path = f"{settings.STORE_DIR}"
    url = downloadStore.url
